<script>
  ...
  let stationsStore = makeWeatherStore();
  let stations = [];
  let rainStore = makeRainStore();
  let weatherPredictions = [];

  $: computedStations = stations.filter((station) => !station.error);
  $: currentRainData = () => {
  ...
  onMount(() => {
    stationsStore.subscribe((data) => {
      stations = data;
    })
    rainStore.subscribe((data) => {
      weatherPredictions = data;
    })
  })

  function tempBelowZero(temp) {
...
<div class="flex flex-col bg-accent-color dark:bg-gray-900 rounded-2xl w-full p-4">
  ...
  {/if}
    <div class="flex flex-wrap">
      {#each computedStations as station}
          <div
