...
<div class="avalanche card">
  {#if warnings.length > 0}
    <!-- If statement added because accessing warnings[4] (today) might happen before warnings are loaded -->
    ...
    <div class="avalanche-today">
      ...
      <div>
        <h3>
          Snøskredvarsel for Trollheimen {formatTimestamp(
            warnings[4].ValidFrom
          )}
        </h3>
        <p>{warnings[0].MainText}</p>
      </div>
    </div>
    <div class="avalanche-border" />
    <div class="avalanche-week">
      <h3>Varsel til uka</h3>
      <p>
        Varsler fra Snøskredvarslingen i Norge og <a
          href="https://varsom.no"
          target="_blank">https://varsom.no</a
        >
      </p>
      <div class="avalanche-circle-container">
        {#each warnings as warning}
          <div
            class={isToday(warning.PublishTime) ? `active warning` : `warning`}
          >
