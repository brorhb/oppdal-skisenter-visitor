<script>
  ...
  const fetchAllAlerts = async () => {
    try {
      alerts = await OFetch(`${config.BASE_URL}/admin/alert/`, 'GET');
      console.log(alerts);
    } catch (error) {
      console.warn(error);
...
<div class="admin-alerts">
  <div class="admin-add">
    ...
    <textarea
      ...
      type="text"
      name="message"
      placeholder="Skriv en oppdatering her (max 150 tegn)"
      maxlength="150"
      bind:value={newAlert}
    />
          ...
          <tr class="admin-table-row">
            <th
              ><p class="table-timestamp">
                {get_publish_date(alert.timestamp)}
              </p></th
            >
            ...
            <th class="toggle-col">
              <div
                class="toggle-text"
                style={alert.is_live ? 'color: #22A830' : 'color: #BABABA'}
              >
                <p>{alert.is_live ? 'Aktiv' : 'Ikke aktiv'}</p>
              </div>
              <div>
                <label class="toggle-switch"
                  ><input
                    type="checkbox"
                    on:change={() => toggleIsLive(alert)}
                    bind:checked={alert.is_live}
                  /><span class="slider" /></label
                >
              </div>
            </th>
            <th on:click={() => (editItem = alert)}
              ><i class="fas fa-edit" /></th
            >
            <th on:click={() => deleteAlert(alert)}
              ><i class="fas fa-trash-alt" /></th
            >
          </tr>
        {/each}
      </tbody>
...
{#if editItem}
  <div class="admin-blur" on:click={() => (editItem = undefined)} />
  <div class="admin-edit">
    ...
    <textarea
      ...
      type="text"
      name="message"
      bind:value={editItem.message}
    />
    <div>
      <button class="admin-button" on:click={() => (editItem = undefined)}
        >Avbryt</button
      >
